...
3. **高低比较**：顺序播放两个十二音 MIDI 音符，选择更高、相同或更低
4. **同音匹配 → 指板位置**：播放目标音，试听三个候选亮点并手动提交相同 MIDI 音高的位置
5. **音程听辨**：入门级使用六种上行音程；进阶级加入小二、小三并随机上下行
6. **大三 / 小三和弦辨别**：播放紧凑排列三和弦，仅判断性质
7. **听音高 → 指板自由定位**：进阶选修；播放一个音，点击任意相同 MIDI 音高的位置
8. **听音高 → 音名 + 指板位置**：播放声音，用户点击音名按钮 + 点击指板位置，两件事都做完后自动提交
9. **和弦 → 指板位置**：显示和弦名，点击真实指法中的发声位置，手动提交
10. **指板按和弦 → 和弦名**：指板出现多个亮点，用户点击和弦名按钮提交
11. **听和弦 → 和弦名**：播放和弦声音，用户点击和弦名按钮提交
