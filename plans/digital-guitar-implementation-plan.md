...
4. **和弦 → 位置**：点击真实指法中的发声位置，包括空弦；闷弦无需选择；手动提交。
5. **亮点和弦 → 和弦名**：显示真实指法中的全部发声位置；选择和弦名。
6. **听和弦 → 和弦名**：播放真实指法发出的 MIDI 音符；选择和弦名。
7. **高低比较**：顺序播放两个十二音 MIDI 音符；选择“更高 / 相同 / 更低”。
8. **同音匹配 → 指板位置**：播放目标音；试听三个候选亮点，手动提交相同 MIDI 音高的位置。
9. **听音高 → 指板自由定位**：进阶选修；播放一个十二音 MIDI 音符，点击任意同 MIDI 指板位置。
10. **音程听辨**：入门级使用六种上行音程；进阶级加入小二度、小三度与随机上下行。
11. **大三 / 小三和弦辨别**：播放随机根音的紧凑排列三和弦；只判断和弦性质。

调弦边界：

- Standard 支持全部练习。
- Drop D、Open G 和自定义调弦支持自由弹奏与音名练习。
- 非 Standard 调弦下禁用和弦练习，并显示原因。
