...
| 指板训练 | 大调指型 → 音名 | 选择大调与 Mi/Sol/La/Ti/Re 指型，在级数形状中识别目标点音名 |
...
- “大调指型 → 音名”支持 12 个大调与 Mi/Sol/La/Ti/Re 五个指型，按 `56 / 712 / 345 / 671 / 234` 级数循环生成形状
- “同音匹配 → 指板位置”提供入门与进阶级别，候选点点击后可试听，手动提交答案
- 练习类型可直接切换，无需返回主界面
- 需要点击指板作答的题型中，点击品位和空弦都会发声
- 非标准调弦下禁用三种和弦练习和大调指型练习
...
- **大调指型题目**：在标准调弦下按所选大调和 Mi/Sol/La/Ti/Re 指型生成级数形状，并从形状内调内音随机选择目标点
- **唯一标识**：每道题有递增的 `id`，确保 React effect 总是触发

### 评判标准

- **音名**：必须完全一致（C 答成 D 算错）
- **大调指型音名**：必须使用当前大调的真实调内音名（例如 D 大调中 F♯ 不能答成 F）
...
| 升降号范围有限 | 只有“大调指型 → 音名”使用当前大调的升降号音名，其他音名题仍以自然音为主 |
