...
4. **参照音 → 指板位置**：播放一个音，点击任意相同 MIDI 音高的位置
5. **音程听辨**：入门级使用六种上行音程；进阶级加入小二、小三并随机上下行
6. **大三 / 小三和弦辨别**：播放紧凑排列三和弦，仅判断性质
7. **听音高 → 音名 + 指板位置**：播放声音，用户点击音名按钮 + 点击指板位置，两件事都做完后自动提交
8. **和弦 → 指板位置**：显示和弦名，点击真实指法中的发声位置，手动提交
9. **指板按和弦 → 和弦名**：指板出现多个亮点，用户点击和弦名按钮提交
10. **听和弦 → 和弦名**：播放和弦声音，用户点击和弦名按钮提交
